...
LeastSquaresApproximate[xs_?VectorQ, ys_?VectorQ, basis_List, "Orthogonal"] :=
  Module[{A = Table[basis[[j]][xs[[i]]], {i, Length[xs]}, {j, Length[basis]}], Q, R},
    {Q, R} = qrHouseholder[N@A];
    laGaussElim[N@R, N@(Transpose[Q] . N@ys), False]  (* upper-triangular, no pivot *)
  ]
...
RemezApproximate[f_, {a_?NumericQ, b_?NumericQ}, n_Integer,
  ...
  Module[{xs, A, rhs, sol, c, E, xExtr, k = 0, sgn},
    xs = Table[(a+b)/2 + (b-a)/2 Cos[(n+1-j) Pi/(n+1)], {j, 0, n+1}] // N;
    Catch[
      While[k++ < maxIter,
        A   = Table[Append[Table[xs[[i]]^j, {j, 0, n}], (-1.)^i], {i, n+2}];
        rhs = f /@ xs // N;
        (* Solve the (n+2)x(n+2) Remez system with our Gaussian elimination *)
        sol = laGaussElim[N@A, N@rhs, True];
        E   = Last[sol]; c = Most[sol];
        If[Abs[E] < tol, Throw[<|"Coefficients" -> c, "Error" -> Abs[E], "Nodes" -> xs|>]];
        sgn = Sign[f[xs[[1]]] - c . Table[xs[[1]]^j, {j, 0, n}]];
        xExtr = Table[
          Module[{r = min1DGoldenSection[
            Function[xx, sgn (-1.)^i (f[xx] - c . Table[xx^j, {j, 0, n}])],
            xs[[i]], xs[[i+1]], tol, 200]},
            r["Point"]],
          {i, n+1}];
        xs = xExtr
